...
> 📌 **关于飞书 / 邮件推送**：当前 v0.3 版本中，飞书消息与邮件推送以**模拟方式**实现，**未接入真实的飞书 Open API 与邮件服务**。系统内"已发送 / 已查看 / 已点击"的漏斗效果完整可见，但消息不会真的送达员工。后续版本会接入真实 API，业务逻辑与界面均无需改动。
...
当前 v0.3 是**模拟实现**，未接入真实的飞书 Open API 与邮件服务（员工不会真的收到消息，只是系统内有"已发送"记录）。生产部署时找 IT 配置消息推送接入即可——员工端、推送中心、漏斗统计这些都不用动。**后续版本会内置真实接入。**
